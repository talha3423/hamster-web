...
const token1 = '7222769243:AAG6OP0K46McTV2oBxewL3hh7HEv_dIAL7A';
const token = '7503354317:AAG6WKtz42cD8X-Tu8NlPap-UtuaZFKpnc0';
const bot = new TelegramBot(token, { polling: true });

const KeysModel = require('../mongo/models/key');
const { search ,Series, getEpisodes, finalData, SecondPage} = require('./search');
...
bot.on('callback_query', async (query) => {
    ...
    if (!chatId) return
      ...
      try {
       ...
       if(res?.data?.episodes[0] == null){
         bot.sendPhoto(chatId, `https://img.nfmirrorcdn.top/poster/h/${id}.jpg`,{parse_mode:'Markdown', caption:` **${res.data.title}**\nRelease:**${res.data.year}**\n\n For Details on How to download click /details`, reply_markup:{
          inline_keyboard:[
            [{text:'Download',callback_data:`7_${id}`}]
          ]
         }})
       }
        else {
     ...
     let res = await getEpisodes(seasonId,id)
     const options = [];
     res?.episodes.forEach((episode)=>{
      options.push([{ text: `Download: ${episode.s}  ${episode.ep}  ${episode.time} Minutes`, callback_data: `6_${id}_${episode.id}` },{text:'Play',url:`https://iosmirror.cc/hls/${episode.id}.m3u8`}])
    })
    
      let page = 2;
      while(res.nextPageShow == 1){
        res = await SecondPage(seasonId,id,page);
        
        page++;
        res.episodes.forEach((episode) => {
          options.push([{ text: `Download: ${episode.s}  ${episode.ep}  ${episode.time} Minutes`, url: `6_${id}_${episode.id}` },{text:'Play',url:`https://iosmirror.cc/hls/${episode.id}.m3u8`}])
        })
      }
     

     bot.sendPhoto(chatId, `https://img.nfmirrorcdn.top/poster/v/${id}.jpg`,{parse_mode:'Markdown',caption:'Choose Episode',reply_markup:{
      inline_keyboard:options
     }})
      ...
      bot.sendPhoto(chatId, `https://img.nfmirrorcdn.top/epimg/150/${seasonId}.jpg`, {
        caption:'Choose Language. \n\n For Details on How to download click /details',
        reply_markup: {
          inline_keyboard: options
        }
      })
      ...
      const res = await finalData(id);
      const data = await Series(id);
      console.log(data)
      const options = [];
      
        options.push([{ text:'Download '+ data.data.d_lang, url: res.videoStreamUrl }])
        options.push([{ text:'Play ', url: `https://iosmirror.cc/hls/${id}.m3u8` }])

        bot.sendMessage(chatId, `\n\nFor Details on How to download click /details. \n\n${data?.data?.title} \n\n${data?.data?.runtime} Choose Language. `, {
