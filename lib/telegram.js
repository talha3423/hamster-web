...
bot.on('callback_query', async (query) => {
    ...
    if (!chatId) return
     ...
     res?.episodes.forEach((episode)=>{
      options.push([{ text: `${episode.s}  ${episode.ep}  ${episode.time} Minutes`, callback_data: `6_${id}_${episode.id}` }])
    })
      ...
      while(res.nextPageShow == 1){
        ...
        page++;
        res.episodes.forEach((episode) => {
          options.push([{ text: `${episode.s}  ${episode.ep}  ${episode.time} Minutes`, url: `6_${id}_${episode.id}` }])
        })
      }
     ...
     bot.sendPhoto(chatId, `https://img.nfmirrorcdn.top/poster/v/${id}.jpg`,{parse_mode:'Markdown',caption:'Choose Episode',reply_markup:{
      ...
      options.push( [{ text: 'Play', url: `https://iosmirror.cc/hls/${seasonId}.m3u8` }])
      ...
      const options = [];
        ...
        options.push([{ text: data.data.d_lang, url: res.videoStreamUrl }])
