...
async function getUserVerified(channelId, userId, key) {
    try {
      ...
      if (user.status == 'member' || user.status == 'creator') {
        ...
        if (!getUserId) {
          const acceptReply = await bot.sendMessage(userId, 'Bot owner has nothing to do with this use at you own risk. \nBot save some of your data.\n\n click /allow to allow.', { reply_markup: { force_reply: true } })
          await bot.onReplyToMessage(userId, acceptReply.message_id, async (msgReply) => {
...
bot.onText(/\/start/, async (msg) => {
    console.log('start');
    if(!getUserVerified(channelId, msg.chat.id, true)) return 
