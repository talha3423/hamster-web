...
async function getUserVerified(channelId, userId) {
  ...
  const commands = [
    ...
    { command: '/Generate My Clone Key', description: 'clone' },
    { command: '/Generate Chain Cube key', description: 'cube' },
    { command: '/Generate Train Miner key', description: 'miner' },
    { command: '/Generate Bike Ride Key', description: 'ride' },
    { command: '/Generate Merge Away', description: 'merge' },
    { command: '/Generate Tweak Race 3D', description: 'race' },
...
bot.onText(/\/key/, async (msg) => {
  console.log('key');
    ...
    const key4 = await KeysModel.findOneAndDelete({ type: 1 });
    const key5 = await KeysModel.findOneAndDelete({ type: 1 });
    const key6 = await KeysModel.findOneAndDelete({ type: 1 });
